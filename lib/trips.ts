...
import { dayCount, eachDate, isoDate, tripStatus, notFound } from './helpers';
import type { Budget, Trip } from './types';

/** Load a trip with its stops, activities and extra costs, shaped for the UI. */
export async function loadTrip(tripId: number | string): Promise<Trip> {
  ...
  const byStop = new Map<number, any[]>(stops.map((s) => [s.id, []]));
 ...
 */
export function budgetFor(trip: any, stops: any[], activities: any[], costs: any[]): Budget {
  const travellers = Math.max(1, trip.travellers || 1);

  const entered: Record<string, number> = { transport: 0, stay: 0, meals: 0, other: 0 };
  for (const c of costs) entered[c.category] = (entered[c.category] || 0) + Number(c.amount);
   ...
   */
  const dateOwner = new Map<string, any>();
  ...
  const perDay = new Map<string, number>(eachDate(trip.start_date, trip.end_date).map((d) => [d, 0]));
  for (const a of activities) {
    const d = isoDate(a.scheduled_date)!;
    if (perDay.has(d)) perDay.set(d, (perDay.get(d) || 0) + Number(a.cost) * travellers);
  }
  for (const [date, stop] of dateOwner) {
    if (perDay.has(date)) perDay.set(date, (perDay.get(date) || 0) + Number(stop.cost_index || 0) * travellers);
  }
...
export async function listTrips(userId: number | string): Promise<Trip[]> {
  const trips = await q(
    `SELECT t.*,
  ...
  return trips.map((t) => ({
    ...
    route: (stopsBy[t.id] ?? []).map((s: any) => ({ id: s.id, city: s.city_name, country: s.country })),
    stops: (stopsBy[t.id] ?? []).map((s: any) => ({
      ...s,
      start_date: isoDate(s.start_date)!,
      end_date: isoDate(s.end_date)!,
      nights: Math.max(0, dayCount(s.start_date, s.end_date) - 1),
      activities: (actsBy[t.id] ?? [])
        .filter((a: any) => a.stop_id === s.id)
        .map((a: any) => ({ ...a, scheduled_date: isoDate(a.scheduled_date)! })),
    })),
    costs: costsBy[t.id] ?? [],
...
export async function ownedTrip(tripId: number | string, userId: number | string): Promise<any> {
  const trip = await one('SELECT * FROM trips WHERE id = ?', [tripId]);
  if (!trip) throw notFound('That trip does not exist.');
  if (trip.user_id !== Number(userId)) throw notFound('That trip does not exist.');
  return trip;
}
