...
/**
 * The search / group / filter / sort row on list screens.
 */
export function ControlStrip({
  ...
  return (
    <div className="card mb-5 overflow-hidden">
          ...
          <button
            ...
            className={`inline-flex h-10 items-center gap-1.5 rounded-[10px] border px-3 text-[13px] font-medium transition-colors ${
              showFilters || filterCount
                ...
                : 'border-rule bg-surface text-slate hover:bg-canvas'
            }`}
          >
            <Filter size={14} />
            {filterLabel ?? 'Filter'}
      ...
      {filterPanel && showFilters && <div className="rise border-t bg-canvas/60 p-3">{filterPanel}</div>}
    </div>
  );
}
